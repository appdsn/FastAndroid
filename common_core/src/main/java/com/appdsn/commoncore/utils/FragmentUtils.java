...
public final class FragmentUtils {
     ...
     * Show fragment.
     *
     * @param fragments The fragment will be show.
     */
    public static void show(@NonNull final Fragment... fragments) {
        if (fragments != null && fragments.length > 0) {
            FragmentManager fm = null;
            for (Fragment show : fragments) {
                putArgs(show, false);
                if (fm == null) {
                    fm = show.getFragmentManager();
                }
            }
            operateNoAnim(fm,
                    TYPE_SHOW_FRAGMENT,
     ...
     * Hide fragment.
     */
    public static void hide(@NonNull final Fragment... fragments) {
        if (fragments != null && fragments.length > 0) {
            FragmentManager fm = null;
            for (Fragment hide : fragments) {
                putArgs(hide, true);
                if (fm == null) {
                    fm = hide.getFragmentManager();
                }
            }
            operateNoAnim(fm,
                    TYPE_HIDE_FRAGMENT,
