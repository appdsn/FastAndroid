...
/**
 * Desc:支持上拉加载，下拉刷新，可以自定义各种动画效果，需要改进的地方：刷新和上拉同时进行的冲突
 * <p>
 *
 * @Author: wangbaozhong
...
//    private int pointId;
    ...
    protected void onAttachedToWindow() {
        ...
        //允许跨布局，用于加一些空状态View等
        mChildContent = mContentView;
        if (mContentView instanceof FrameLayout) {
            View firstContent = ((FrameLayout) mContentView).getChildAt(0);
            if (firstContent instanceof AbsListView
                    || firstContent instanceof ScrollingView
                    || firstContent instanceof ScrollView
                    || firstContent instanceof WebView) {
                mChildContent = firstContent;
            }
        }
...
//                float yDistance = Math.abs(dy);
    ...
    public boolean isHeadViewPullOut() {
        boolean isOut = false;
    ...
    public boolean isFootViewPullOut() {
        boolean isOut = false;
    ...
    public void finishLoadMore(boolean hasMoreData) {
        if (mContentView != null) {
            ...
            if (footerStatus == STATUS_REFRESH) {
                postDelayed(new Runnable() {
                    @Override
                    public void run() {
    ...
    public void finishRefresh() {
        if (mContentView != null) {
            ...
            hasMoreData = true; /*重置footerView的状态数据*/
    ...
    private boolean canChildScrollUp() {
        ...
        if (Build.VERSION.SDK_INT < 14) {
            if (mChildContent instanceof AbsListView) {
                final AbsListView absListView = (AbsListView) mChildContent;
                return absListView.getChildCount() > 0
                        && (absListView.getFirstVisiblePosition() > 0 || absListView
                        .getChildAt(0).getTop() < absListView
                        .getPaddingTop());
            } else {
                return mChildContent.canScrollVertically(-1)
                        || mChildContent.getScrollY() > 0;
            }
        } else {
            return mChildContent.canScrollVertically(-1);
        }
