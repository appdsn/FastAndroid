 ...
 */
package com.appdsn.commoncore.http.baseurl.parser;

import android.text.TextUtils;

import com.appdsn.commoncore.http.baseurl.BaseUrlManager;
import com.appdsn.commoncore.http.baseurl.cache.Cache;
import com.appdsn.commoncore.http.baseurl.cache.LruCache;
 ...
 * 比如:
 * 1.
 * 旧 URL 地址为 https://www.github.com/wiki/part, 您在 App 初始化时传入 {@link BaseUrlManager#startAdvancedModel(String)}
 * 的 BaseUrl 为 https://www.github.com/wiki
 * 您调用 {@link BaseUrlManager#putDomain(String, String)} 方法传入的 URL 地址是 https://www.google.com/api
 * 经过本解析器解析后生成的新 URL 地址为 http://www.google.com/api/part
 * <p>
 * 2.
 * 旧 URL 地址为 https://www.github.com/wiki/part, 您在 App 初始化时传入 {@link BaseUrlManager#startAdvancedModel(String)}
 * 的 BaseUrl 为 https://www.github.com/wiki
 * 您调用 {@link BaseUrlManager#putDomain(String, String)} 方法传入的 URL 地址是 https://www.google.com
...
public class AdvancedUrlParser implements UrlParser {
    ...
    @Override
    public void init(BaseUrlManager baseUrlManager) {
    ...
    public HttpUrl parseUrl(HttpUrl domainUrl, HttpUrl url) {
        ...
        if (TextUtils.isEmpty(mCache.get(getKey(domainUrl, url)))) {
            ...
            if (url.pathSize() > mBaseUrlManager.getPathSize()) {
                List<String> encodedPathSegments = url.encodedPathSegments();
                for (int i = mBaseUrlManager.getPathSize(); i < encodedPathSegments.size(); i++) {
                    newPathSegments.add(encodedPathSegments.get(i));
                }
            } else if (url.pathSize() < mBaseUrlManager.getPathSize()) {
                throw new IllegalArgumentException(String.format("Your final path is %s, but the baseUrl of your BaseUrlManager#startAdvancedModel is %s",
                        url.scheme() + "://" + url.host() + url.encodedPath(),
                        mBaseUrlManager.getBaseUrl().scheme() + "://"
                                + mBaseUrlManager.getBaseUrl().host()
                                + mBaseUrlManager.getBaseUrl().encodedPath()));
            }
