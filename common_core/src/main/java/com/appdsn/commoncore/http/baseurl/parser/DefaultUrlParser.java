 ...
 * 然后通过 {@link BaseUrlManager#setUrlParser(UrlParser)} 配置给框架, 即可替换 {@link DefaultUrlParser}
...
public class DefaultUrlParser implements UrlParser {
    ...
    public void init(BaseUrlManager baseUrlManager) {
        ...
        this.mDomainUrlParser.init(baseUrlManager);
    }
    ...
    public HttpUrl parseUrl(HttpUrl domainUrl, HttpUrl url) {
        ...
        if (url.toString().contains(BaseUrlManager.IDENTIFICATION_PATH_SIZE)) {
            if (mSuperUrlParser == null) {
                synchronized (this) {
                    if (mSuperUrlParser == null) {
                        mSuperUrlParser = new SuperUrlParser();
                        mSuperUrlParser.init(mBaseUrlManager);
                    }
                }
            }
        ...
        //如果是高级模式则使用高级解析器
        if (mBaseUrlManager.isAdvancedModel()) {
            if (mAdvancedUrlParser == null) {
                synchronized (this) {
                    if (mAdvancedUrlParser == null) {
                        mAdvancedUrlParser = new AdvancedUrlParser();
                        mAdvancedUrlParser.init(mBaseUrlManager);
                    }
                }
            }
